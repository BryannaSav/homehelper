...
def api_one_day(request, date):
    ...
    day_events = CalendarEvent.objects.filter(start_date__year=year, start_date__month=month,start_date__day=day).values()
    day_tasks = ListItem.objects.filter(due_date__year=year, due_date__month=month,due_date__day=day).values()
